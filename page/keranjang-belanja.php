...
<section id="cart" class="mb-5">
  <div class="container-xl container-md container-sm container container-md container-sm container mt-4">
    <header class="header-keranjang w-100 d-flex justify-content-between">
      <h2>Keranjang Belanja</h2>
      <h4 class="fw-normal align-self-center"><a href="home" class="text-decoration-none">Lanjut Belanja</a></h4>
      <!-- <hr> -->
    </header>
    <section class='keranjang-belanja mt-1'>
    <?php
      $jml = 0;
          ...
          <section class='d-flex justify-content-center keranjang-belanja text-center'>
            <div class='col-lg-4 col-md-6 col-sm-8 col-8 mb-5'>
              <svg id='ffc6eb9a-0ec0-429c-85a8-ff38b44048bf' data-name='Layer 1' xmlns='http://www.w3.org/2000/svg'
                max-width='300' max-height='250' viewBox='0 0 896 747.97143'>
                ...
                <path
                  ...
                  transform='translate(-152 -76.01429)' fill='#6c63ff' />
                ...
                <rect x='385.55745' y='570.79732' width='186.92877' height='1.70379'
                  transform='translate(-274.73922 936.23495) rotate(-86.24919)' fill='#2f2e41' />
                <rect x='334.45728' y='478.18483' width='1.70379' height='186.92877'
                  transform='translate(-188.46866 -52.99638) rotate(-3.729)' fill='#2f2e41' />
              ...
              <div class='mt-3'>
                <a href='home'><button class='btn btn-primary rounded'>Belanja Sekarang</button></a>
              </div>
            </div>
          </section>
          ";
      ...
      <div class='row'>
        ...
        <div class='col-4 nama-produk'>
          ...
          <p class='fs-5 fw-bolder m-0'><?= number_format($harga_total)?>/0.5kg</p>
        </div>
        <div class='col-3 jml-produk mb-2'>
          <div class='kuantiti-produk text-center d-flex flex-row'>
            <form action="kurangkeranjang" method="POST">
              <input type="hidden" name="id" value="<?php echo $produk['id_produk']?>" />
              <!-- <input type="hidden" name="satuan" value="<?php echo $produk['satuan']?>" /> -->
              <button type="submit" class="btn p-0 btn-link">
                <i class='fs-3 text-primary fas fa-minus-circle'></i>
              </button>
            </form>
            <span class=' fs-5 mx-2 jumlah'><?= $val?></span>
            <form action="tambahkeranjang" method="POST">
              <input type="hidden" name="id" value="<?php echo $produk['id_produk']?>" />
              <!-- <input type="hidden" name="satuan" value="<?php echo $produk['satuan']?>" /> -->
              <button class ="btn p-0 btn-link">
                <i  class='fs-3 text-primary fas fa-plus-circle'></i>
              </button>
            </form>
          </div>
        ...
        <div class='col-2 '>
          <div class='text-center'>
            <a href="hapuskeranjang-<?= $produk['id_produk']?>"><i class='fs-3 text-danger fas fa-trash-alt'></i></a>
          </div>
          <!-- <p class='text-center fs-6'>Hapus Ikan</p> -->
        </div>
      </div>
      <?php
    ...
    <footer class="d-flex flex-column">
      ...
      <p class="text-muted pesan"><i class="fas fa-exclamation-circle"></i> Belum termasuk biaya antar</p>
      <?php
        if (isset($_SESSION['items'])) {
          if (count($_SESSION['items']) > 0 ) {?>
          <a class="w-100" href="checkout">
            <button class="w-100 btn btn-dark">Checkout</button>
          </a>
          <?php }else{?>
            <button class="w-100 btn btn-dark disabled">Checkout</button>
          <?php }
        }?>
    </footer>
  </div>
...
<!-- <script type="text/javascript">
  window.addEventListener('DOMContentLoaded', (event) => {
    ...
    var url = "keranjangAjax";
    xhr.onreadystatechange = function(){
      if(this.readyState == 4 && this.status == 200){
        document.getElementById("keranjang").innerHTML = this.responseText;
      }
    };
  ...
  //   var xhr = new XMLHttpRequest();
  //   var url = "keranjangTambah";

  //   var data = {
  //       id_produk: document.getElementById("title").value,
  //       jumlah: document.getElementById("body").value,
  //       userId: 
  //   };
